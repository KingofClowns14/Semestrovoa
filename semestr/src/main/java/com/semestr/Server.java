...
public class Server {
    ...
    // synchronized гарантирует, что записи не перемешаются при одновременных
    // событиях
    ...
    private synchronized void endGame(ClientHandler loser) {
        ...
        statsManager.updateStats(winner.nickname, loser.nickname);
        log("Обновленна Статистика");
        log(winner.nickname + " : " + statsManager.getStatsString(winner.nickname));
        log(loser.nickname + " : " + statsManager.getStatsString(loser.nickname));
        log("------------------------------------------");
    }
            ...
            try {
                ...
                while ((line = in.readLine()) != null) {
                    ...
                    if (line.startsWith("NICK")) {
                        ...
                        String stats = statsManager.getStatsString(this.nickname);
                        log("Игрок #" + id + " представился как: " + nickname);
                        log("Статистика: " + stats);
                        broadcast(line, this);
